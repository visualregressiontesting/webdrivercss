...
module.exports = function (imageDiff, done) {
    ...
    if (this.misMatchTolerance < misMatchTolerance) {
        ...
        mkdirp.sync(path.dirname(this.diffPath));
        imageDiff.getDiffImage().pack().on('end', done.bind(null, null, this.resultObject)).pipe(fs.createWriteStream(this.diffPath));
    } else {
