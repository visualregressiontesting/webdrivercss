...
module.exports = function documentScreenshot(fileName) {
    ...
    var self = this.instance,
        ...
        currentYPos = 0,
        screenshot = null,
        scrollFn = function scrollFn(w, h) {
        /**
         * IE8 or older
         */
    ...
    function (res, cb) {
        ...
        function (finishedScreenshot) {
             ...
             */
            function (cb) {
                self.screenshot.call(self).then(function (res) {
                    return cb(null, res);
                }).catch(function (err) {
                    return cb(err);
                });
            ...
            function () {
                ...
                self.execute(scrollFn, currentXPos * response.execute[0].value.screenWidth, currentYPos * response.execute[0].value.screenHeight).then(function (res) {
                    response.execute.push(res);
     ...
     */
    function (cb) {
        self.execute(scrollFn, 0, 0).then(function () {
            cb();
        });
    }], function (err) {
        if (err) {
